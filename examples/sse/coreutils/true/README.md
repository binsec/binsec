...
- target executable:    sysroot/bin/true
- debug information:    sysroot/usr/lib/debug/.build-id/0e/ea10bd32e512b82055e32b8cb1b9b3e46e4d89.debug   
- SSE script:           [run.ini](./run.ini)

# Goal

Output the trace execution of the `true` command (do nothing, successfully) from a sysroot archive.
Use the target filesystem with the `-sse-sysroot` option to retrieve debug symbols from the standard location even if the binary has been stripped.
