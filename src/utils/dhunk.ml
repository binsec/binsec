...
module C = struct
  module C_dot = struct
    ...
    let vertex_name v =
      Format.asprintf "\"%d: %a\"" (V.addr v)
        (Print_utils.pp_opt Dba_printer.Ascii.pp_instruction)
        (V.inst v)
...
let node g n = C.mem_vertex_a g (A.of_int n)

let inst g n = match node g n with None -> None | Some v -> C.V.inst v

let add g idx instr = C.add_inst g (A.of_int idx) (I.of_instr instr)
...
let start g = match C.mem_vertex_a g 0 with None -> assert false | Some v -> v
...
let fold f acc g =
  C.fold_vertex
    (fun v acc -> match C.V.inst v with None -> acc | Some i -> f acc i)
    g acc
...
let flatten g =
  C.fold_vertex
    (fun v acc ->
      match C.V.inst v with None -> acc | Some i -> (C.V.addr v, i) :: acc)
    g []
...
let iteri ~f g =
  C.iter_vertex
    (fun v ->
      let a = C.V.addr v in
      match C.V.inst v with None -> () | Some i -> f a i)
    g
...
let pp_succs node_addr ppf succ_nodes =
  ...
  let succ_addrs =
    List_utils.filter_map (( <> ) direct_succ) C.V.addr succ_nodes
  in
  ...
  List.iter
    (fun v ->
      ...
      | Some ((DJump _ | SJump _ | If _) as inst) ->
          ...
          fprintf ppf "@[<h>%2d: %a@]@ " addr Dba_printer.Ascii.pp_instruction
            inst
      | Some inst ->
          let addr = C.V.addr v in
          let next_addrs = succ t v in
          fprintf ppf "@[<h>%2d: %a%a@]@ " addr Dba_printer.Ascii.pp_instruction
            inst (pp_succs addr) next_addrs)
    g_l;
  fprintf ppf "@]"
...
let is_return g =
  let rec aux node =
    ...
    | Some dinst -> (
        DI.is_return dinst
        || match C.succ g node with [ v ] -> aux v | _ -> false)
  in
  match C.mem_vertex_a g (A.of_int 0) with None -> false | Some v -> aux v
  ...
  let no_temporary_leak g =
    ...
    Logger.debug ~level:6 "@[<v 0>%a@]"
      (fun ppf g ->
        C.iter_vertex
          (fun v ->
            Format.fprintf ppf "%d: %a [%a]@ " (C.V.addr v)
              (Print_utils.pp_opt Dba_printer.Ascii.pp_instruction)
              (C.V.inst v)
              (fun ppf s ->
      ...
      C.iter_vertex
        (fun v ->
          ...
          | Some inst ->
              ...
              if not reachable then C.remove_vertex g v
              else
    ...
    C.iter_vertex
      (fun v ->
        ...
        | Some (Instr.SJump (JInner _, _) as just_i) -> (
            ...
            | [ s ], [ p ] ->
                Logger.debug "Removing %a" Dba_printer.Ascii.pp_instruction
                  just_i;
                C.add_edge g p s;
                C.remove_edge g p v;
                C.remove_edge g v s;
                ...
                | Some i ->
                    ...
                    let instr = DI.reset_successor ~src_id ~dst_id i in
