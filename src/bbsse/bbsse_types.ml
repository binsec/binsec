...
module Path = struct
  ...
  (* Print tout le path dans un fichier *)
...
module Path_state = struct
  type t = {
    ...
    solver_calls : int;
    path : Path.t;
    (* Sequence of DBA statements for this path *)
    ...
    address_counters : Bbsse_options.Address_counter.t Virtual_address.Map.t;
    cfg : C.t;
    (* The CFG that we are --- possibly --- building *)
  ...
  let decode addr ps =
    ...
    | Some v -> (
        ...
        | Some _, None ->
            let msg =
              Format.asprintf "Error 1 durring sse in CFG construction %@ %a"
                Virtual_address.pp addr
            in
            failwith msg
        | None, Some _ ->
            let msg =
              Format.asprintf "Error 2  durring sse in CFG construction %@ %a"
                Virtual_address.pp addr
            in
            failwith msg
        | Some i, Some c ->
            (* Link to its predecessor the returned instruction *)
            C.add_symb cfg addr (c + 1);
            link v ps;
            i)
  ...
  let set_instruction instruction st = { st with instruction }
  ...
  let with_init_mem_at ~addr ~size path_state =
    let symbolic_state =
      ...
      Sse_symbolic.State.init_mem_at path_state.symbolic_state ~addr ~size
    in
  ...
  let is_depth_ok ps =
    let max_depth = Bbsse_options.MaxDepth.get () in
    ps.depth < max_depth
  ...
  let do_optimization ?(keep = Formula.VarSet.empty) ~universals fm =
    let level = 3 in
    ...
    if Formula.VarSet.is_empty universals then
      ...
      Logger.debug ~level:3 "@[<hov 2>\\forall variables: %a@]"
        Formula_pp.pp_varset universals;
    (*let is_controlled =
      let open Formula in
      if VarSet.is_empty universals then None
      else Some (fun var -> not @@ VarSet.mem var universals) in*)
    Formula_transformation.optimize ~keep fm
  ...
  let create ?(depth = 0) ?(address_counters = Virtual_address.Map.empty) ?cfg
    ...
    let cfg =
      match cfg with
      | None -> C.create (Bbsse_options.MaxDepth.get ())
      | Some cfg -> cfg
    in
    ...
    let ps =
      {
        ...
        instruction;
        solver_calls = 0;
        (* At path creation we have never called a solver *)
        cfg;
        entrypoint;
      }
    in
    ...
    let count =
      match Virtual_address.Map.find entrypoint address_counters with
      | exception Not_found -> Address_counter.init entrypoint 1
      | c -> c
    in
    C.add_symb ps.cfg entrypoint count.counter;
    ps

  let dump_cfg ~filename ps = C.dump ~filename ps.cfg
...
module G (W : WORKLIST) = struct
  ...
  type t = {
    worklist : W.t;
    directives : d;
    (* utilisé pour enumerate :
       * si ona deja listé ( par ex) 4 valeurs : 1 2 3 4
       * On va en lsiter d'autres
    *)
    enumerations : Bv_set.t Etbl.t Virtual_address.Htbl.t;
  }

  let get_next_path e = fst (W.pop e.worklist)
  ...
  module Directives = struct
    ...
    let address d = Directive.addr d

    let count e = e.directives.todo

    let incr_todo e = e.directives.todo <- e.directives.todo + 1
    ...
    *)
    ...
    let get_store e = e.directives.store

    let extend_directive store vaddr d =
      Logger.debug ~level:2 "Extend %a with directive %a" Virtual_address.pp
        vaddr Directive.pp d;
    ...
    let init () =
      let todo = ref 0 in
      let gs = Sse_options.Directives.get () in
      ...
      let add_actions gs =
        List.iter
          (fun g ->
            ...
            | Choice _ | Cut _ | Assert _ | Assume _ -> ()
            | Reach _ | Enumerate _ -> incr todo);
            Logger.debug ~level:2 "Add action %a" Directive.pp g;
    ...
    module Enumeration = struct
      ...
      let record va e bvs g =
        ...
        | Values (vs, h) ->
            ...
            Etbl.replace h e v'
        | No_values h -> Etbl.add h e bvs
        | No_record ->
            let etbl = Etbl.create 3 in
            Etbl.add etbl e bvs;
            Virtual_address.Htbl.add g.enumerations va etbl

      (* retourne le nb d'elt du bv_set(va;e) *)
      let count va e g =
        match access va e g with
        | No_record | No_values _ -> 0
  ...
  let from_address ?(block_index = 0) ~initialize_fun ~entrypoint =
    let level = 4 in
    ...
    let initial_path_state =
      Path_state.create ~block_index symbolic_state initial_instruction
      |> initialize_fun
    in
  ...
  let reinitialize ?(block_index = 0) ~initialize_fun ~entrypoint former_g =
    let level = 4 in
    let initial_instruction = decode entrypoint in
    Logger.debug ~level "Creating symbolic store ...";
    let symbolic_state = Sse_symbolic.State.create () in
    ...
    Logger.debug ~level "@[<h>Initializing SSE goals ...@]";
    let directives = Directives.init () in
    let enumerations = former_g.enumerations in
    { worklist; directives; enumerations }
...
module type GLOBAL_ENV = sig
  ...
  val get_next_path : t -> Path_state.t
  ...
  module Directives : sig
    ...
    val get_store : t -> Directive.t Queue.t Virtual_address.Htbl.t

    val extend_directive :
      Directive.t Queue.t Virtual_address.Htbl.t ->
      Virtual_address.Htbl.key ->
      Directive.t ->
      unit
    ...
    module Enumeration : sig
      val record :
        Virtual_address.t -> Dba.Expr.t -> Bitvector.t list -> t -> unit
