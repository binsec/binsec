...
module Query_stats = struct
  type t = { unsat : int; sat : int; err : int; enums : int; time : float }

  let empty = { unsat = 0; sat = 0; err = 0; time = 0.; enums = 0 }

  let total q = q.unsat + q.sat + q.err

  let avg_time q = q.time /. (float @@ total q)

  let add_enumeration q = { q with enums = q.enums + 1 }
  ...
  let add q time res =
    ...
    | Formula.TIMEOUT | Formula.UNKNOWN -> { q with err = q.err + 1 }
  ...
  let pp ppf q =
    ...
    fprintf ppf
      "@[<v 0>@[<h>sat            %d@]@,\
       ...
       @[<h>average        %.2f@]@,\
       @]"
      q.sat q.unsat q.err (total q) q.enums q.time (avg_time q)
...
module Solver = struct
  ...
  let with_solver ?keep path_state f =
    ...
    let file =
      if Sse_options.SMT_dir.is_set () then (
        ...
        Logger.debug ~level:3 "@[<h>Using SMT script file %s %@ %a@]" filename
    ...
    Logger.debug ~level:5 "Running %s %@ %a (file %a)" (Prover.name_of solver)
  ...
  let get_addresses_to_load session path_state =
    if not (LoadSections.is_set () || LoadROSections.get ()) then no_address
    else
  ...
  let maybe_load_and_recurse f result path_state to_load =
    if S.is_empty to_load then (result, path_state)
      ...
      Logger.debug ~level:1 "at %a:@ loading addresses @ %a" Path_state.pp_loc
        path_state
        (fun ppf ->
  ...
  let rec check_satistifiability path_state =
    let result, to_load =
      with_solver path_state (fun session ->
  ...
  let get_model path_state =
    let f session =
      ...
      | status ->
          Logger.error "Asking for a model on a %a formula" Formula_pp.pp_status
            status;
          exit 1
    in
    match with_solver path_state f with None -> None | Some (_, r) -> Some r
  ...
  let rec enumerate_values n expr path_state =
    ...
    let rec loop acc to_load n solver =
      ...
      | _ -> (
          ...
          | Formula.SAT ->
              let bv =
                extract_bv
                  (Solver.Session.get_value solver (Formula.mk_bv_term expr))
              in
          ...
          | res ->
              Logger.debug ~level:4 "Solver returned %a" Formula_pp.pp_status
                res;
              (acc, to_load))
    in
    ...
    *)
    ...
    let values, to_load =
      with_solver ~keep path_state (loop [] no_address n) |> function
      | None -> ([], no_address)
      | Some (_, v) -> v
    in
    ...
    if List.length values = n then
      Logger.warning
        "Found as many solutions for@ %a@ as asked.@ Possibly incomplete \
         solution set."
...
module Translate = struct
  ...
  let nondet ?naming_hint ?(wild = false) lvalue path_state =
    ...
    let symstate =
      Sse_symbolic.State.declare ~wild name (Formula.BvSort size) symstate
    in
    let logical_rval_base, st =
      Sse_symbolic.State.get_bv name (Size.Bit.create size) symstate
    in
    let name, var_type, logical_rval, st' =
      lvalue_with_rval_update st logical_rval_base lvalue
    in
    let st' = Sse_symbolic.State.assign name var_type logical_rval st' in
  ...
  let assume cond state =
    ...
    Path_state.set_symbolic_state st state |> Path_state.add_assertion tr_cond
end
