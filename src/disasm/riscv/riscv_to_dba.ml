...
module D = struct
  ...
  let ( <-- ) dst src = if Dba.Expr.is_equal dst src then Nop else Asg (dst, src)
...
module Lift = struct
  ...
  let jalr st ~dst ~src ~offset =
    ...
    let sr =
      if is_x0 r then empty
        ...
        ini (r <-- next)
    in
    let dba = !!(sr +++ ejmp (De.add (reg_bv src) (mk_imm offset))) in
    let mnemonic =
      if is_x0 r then "ret"
      else
  ...
  let lui st ~dst ~offset =
    ...
    in
    let mnemonic =
      Printf.sprintf "lui %s,%s" (reg_name dst) (Bitvector.to_hexstring offset)
    in
    (mnemonic, dba)
  ...
  module C = struct
    ...
    let li st ~dst ~imm5 ~imm1 =
      ...
      ( Printf.sprintf "li %s,%s" (reg_name dst) (Z.to_string (Bv.signed_of imm)),
        dba )
    ...
    let lift_to_uncompressed bop st ~dst ~src = bop st ~dst ~src1:dst ~src2:src
...
module I = struct
  ...
  module Stype = struct
    ...
    let slli st bits =
      ...
      apply_shift "slli" Lift.slli st ~shamt:s.rs2 ~dst:s.imm5 ~src:s.rs1
        ~opcode:bits
    ...
    let srxi st opcode =
      ...
      let name, lift_f =
        ...
        | 0 -> ("srli", Lift.srli)
        | 0x20 -> ("srai", Lift.srai)
        | _ -> fail "Unexpected shift right operation"
      in
  ...
  module C = struct
    ...
    let c0 st opcode =
      ...
      | 2 ->
          ...
          ins @@ Inst.create ~mnemonic ~dba ~opcode
      | 3 -> if Riscv_arch.Mode.is_m32 mode then unh "C.flw" else unh "C.ld"
      | 4 -> unh "C.reserved"
