...
let remove_goto m =
  ...
  let rec f addr (*(ca, id)*) (ik, opcode) accu =
    ...
    | Dba.Instr.Nondet (_, _, nid) -> (
        ...
        with Not_found -> Caddress.Map.add addr (ik, opcode) accu)
        ...
        try
          ...
          | _ ->
              if addr.Dba.id = 0 then
                let accu =
                  Caddress.Map.add a
                    (Instruction.set_successor ik 0, nopcode)
                    accu
                in
    ...
    | Dba.Instr.If (cond, Dba.JOuter addr' (*(nca, nid1)*), nid2) -> (
        try
          ...
          | Dba.Instr.SJump (Dba.JInner nnid, _) ->
              let inst =
                Dba.Instr.ite cond (Dba.Jump_target.outer addr') nnid
              in
    ...
    | Dba.Instr.If (cond, Dba.JInner nid1, nid2) -> (
        ...
        let nik1 =
          try
            let elt1 = local_find a1 in
            Some (fst elt1)
          with Not_found -> None
        in
        let nik2 =
          try
            let elt2 = local_find a2 in
            Some (fst elt2)
          with Not_found -> None
        in
        ...
        | Some nik, None -> (
            ...
            | Dba.Instr.SJump (Dba.JInner nnid, _) ->
                let inst =
                  Dba.Instr.ite cond (Dba.Jump_target.inner nnid) nid2
                in
                f addr (inst, opcode) accu
            | _ -> Caddress.Map.add addr (ik, opcode) accu)
        | None, Some nik -> (
            ...
            | Dba.Instr.SJump (Dba.JInner nnid, _ntag) ->
                let inst =
                  Dba.Instr.ite cond (Dba.Jump_target.inner nid1) nnid
                in
                f addr (inst, opcode) accu
            | _ -> Caddress.Map.add addr (ik, opcode) accu)
        | Some ik1, Some ik2 -> (
            let as_ite inner_id succ =
              Dba.Instr.ite cond (Dba.Jump_target.inner inner_id) succ
            in
            match (ik1, ik2) with
