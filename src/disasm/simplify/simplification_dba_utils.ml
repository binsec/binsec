...
let statistics instMap =
  let f _ (ik, _) (accusize, accugoto, temp, flag) =
    match ik with
    | Dba.Instr.SJump (JOuter _, _) -> (accusize + 1, accugoto + 1, temp, flag)
  ...
  in
  Dba_types.Caddress.Map.fold f instMap (0, 0, 0, 0)
...
let rec lhs_mayused_in_expr lhs expr =
  match (lhs, expr) with
  | _, Dba.Expr.Cst _ | Dba.LValue.Store _, Dba.Expr.Var _ -> false
  | ( Dba.LValue.Restrict
        ({ name = name1; _ }, { Interval.lo = i1; Interval.hi = j1 }),
      Dba.Expr.Unary
        ( Dba.Unary_op.Restrict { Interval.lo = i2; Interval.hi = j2 },
          Dba.Expr.Var v ) ) ->
...
let rec is_not_mayused prog addr look_ahead_limit var flags_env :
  ...
  if look_ahead_limit = 0 then (flags_env, false)
    (* sliding window ends without mustkill proof *)
  else
    try
...
and is_not_mayused_in_instr ik prog addr niter var flags_env :
  ...
  | Dba.Instr.Stop _ -> (flags_env, true)
(* if not used then useless *)
...
and is_not_mayused_in_call prog addr lhs flags_env :
  ...
  | Dba.LValue.Var { name; _ } -> (
      ...
      | _ -> (
          match
            Basic_types.String.Map.find name
          ...
          | exception Not_found ->
              ...
              let sub_flags_env =
                ...
                with Not_found -> Basic_types.String.Map.empty
              in
              ...
              (flags_env, b)))
  | Dba.LValue.Restrict _ | Dba.LValue.Store _ -> (flags_env, false)
(* lhs is a flag = cannot be a store *)
...
let display_results (initsize, _, itemps, iflags) map ppf time =
  ...
  in
  Format.fprintf ppf
    "@[<v 0>Simplification statistics (%.2fs):@ Disassembled instructions : \
     %d@ Generated dba instructions : %d@ Simplification : %a@ Temporaries \
     simplification : %a@ Flags simplification : %a@ @] "
    time
    (X86toDba.native_instructions_decoded ())
    finalsize (pp_ratio initsize) finalsize (pp_ratio itemps) ftemps
