...
module Engine = struct
  ...
  let () =
    register "default" Default (fun () ->
        (module Symbolic.Default.State : Symbolic.State.S))

  module Opt = Builder.Any (struct
    type nonrec t = t
    ...
    let of_string name =
      ...
      with Not_found ->
        if List.mem name (Cli.Plugins.list ()) then (
  ...
  let get_factory () =
    match Htbl.find factories (Opt.get ()) with
    | _, f -> f ()
    | exception Not_found ->
...
module QMerge = Builder.False (struct
  let name = "qmerge"
  let doc = "Enable quick merging"
end)
...
module Sysroot = Builder.String_option (struct
  let name = "sysroot"

  let doc =
    "specifies the local directory that contains copies of target libraries in \
     the corresponding subdirectories. "
end)

type warnerror = Exec.warnerror = Error | Warn | Quiet
