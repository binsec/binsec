...
module Make (E : Metrics.EXPLORATION) (Q : Metrics.QUERY) = struct
  ...
  let draw state =
    if not state.drawn then (
      ...
      state.drawn <- true);
    let total_time = E.Timer.get () in

    let paths = E.Paths.get Total
    and pending = E.Paths.get Pending
    and completed = E.Paths.get Completed
    and discontinued = E.Paths.get Discontinued
    and branches = E.Topology.get Branch
    and total_asserts = E.Topology.get Assert
    and failed_asserts = E.Paths.status Assertion_failure
    and max_depth = E.Max_depth.get ()
    and instructions = E.Instructions.get ()
    and unique_insts = E.Addresses.unique ()
    and pre_sat = Q.Preprocess.get Sat
    and pre_unsat = Q.Preprocess.get Unsat
    and pre_time = Q.Preprocess.Timer.get ()
    and sol_sat = Q.Solver.get Sat
    and sol_unsat = Q.Solver.get Unsat
    and sol_unknown = Q.Solver.get Unknown in
    let sol_time = pre_time +. Q.Solver.Timer.get () in

    Draw.int 2 25 paths;
    Draw.int 3 25 pending;
    Draw.int 4 25 completed;
    Draw.int 5 25 discontinued;
    ...
    Draw.int 6 55 (pre_sat + pre_unsat);
    Draw.int 7 55 pre_sat;
    Draw.int 8 55 pre_unsat;
    Draw.time 9 55 pre_time;

    Draw.int 12 55 (sol_sat + sol_unsat + sol_unknown);
    Draw.int 13 55 sol_sat;
    Draw.int 14 55 sol_unsat;
    Draw.int 15 55 sol_unknown;
...
let () =
  Plugins.register ~is_enabled:Options.Monitor.get (fun () ->
      (module struct
        let name = "screen"
        let fields _ = []
        ...
        let extensions :
         ...
         fun engine ->
          ...
          let module Screen =
            Make (Engine.Metrics.Exploration) (Engine.Metrics.Queries)
          in
          [
            Initialization_callback (fun _ -> Screen.init ());
            Exit_callback (fun () -> Screen.release ());
          ]
      end : Types.PLUGIN))
