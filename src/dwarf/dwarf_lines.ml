...
module Opcode = struct
  ...
  let load_extended cursor size : t =
    ...
    | 0x01 -> End_sequence
    | 0x02 ->
        if size = 5 then Set_address (Read.u32 cursor)
        else if size = 9 then Set_address (Read.u64 cursor)
        else
          Dwarf_options.Logger.fatal
...
module Header = struct
  ...
  let load cursor : t =
    ...
    if version <> 2 then
      Dwarf_options.Logger.fatal "trying to read Line header with version <> 2";
    ...
    let standard_opcode_lengths =
      Array.init opcode_base (fun i -> if i = 0 then 0 else Read.u8 cursor)
    in
    let rec loop include_directories =
      match Peek.u8 cursor with
      | 0x00 ->
          advance cursor 1;
    ...
    while Peek.u8 cursor <> 0 do
      Hashtbl.add file_names (Hashtbl.length file_names + 1) (File.load cursor)
    done;
...
let decode vm hd lines = function
  | Opcode.Special (a, l) ->
      ...
      vm.Machine.current.Machine.address <-
        vm.Machine.current.Machine.address + (hd.Header.min_instr_length * a);
  ...
  | Opcode.Advance_pc a | Opcode.Const_add_pc a ->
      vm.Machine.current.Machine.address <-
        vm.Machine.current.Machine.address + (hd.Header.min_instr_length * a);
      lines
  ...
  fun ppf l ->
    ...
    List.iter
      (fun { path; line; addresses = { Interval.lo; _ }; _ } ->
        Prettytbl.append t
          [| path; string_of_int line; Format.sprintf "%#x" lo |])
      l;
    Prettytbl.pp ppf t;
    Format.pp_close_box ppf ()
