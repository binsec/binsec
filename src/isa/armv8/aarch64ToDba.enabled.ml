...
let decode_from_reader (addr : Virtual_address.t) reader =
  if Virtual_address.modi addr 4 <> 0 then
    ...
    match Reader.Read.i32 reader with
    ...
    | bytes ->
        ...
        (match status with
        | None ->
            Reader.rewind reader (4 - Size.Byte.to_int ins.size);
            Parser.incr_success ins.opcode
        | Some Undefined ->
            Reader.rewind reader 4;
...
let () = Decoder.register (Machine.armv8 LittleEndian) decode
