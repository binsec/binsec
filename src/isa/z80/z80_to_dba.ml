...
let decode_cb reader =
  let op = Uint8.to_int (Reader.Read.u8 reader) in
...
let decode_ddcb reader =
  let d = Int8.to_int (Reader.Read.i8 reader) in
  let op = Uint8.to_int (Reader.Read.u8 reader) in
  match Char.unsafe_chr op with
  | '\x00' .. '\x05'
  | '\x07' .. '\x0d'
...
let decode_dd reader =
  ...
  | '\x26' | '\x2e' ->
      LD8.ld_p_n ((op lsr 3) land 0b111) (Uint8.to_int (Reader.Read.u8 reader))
  | '\x09' | '\x19' | '\x29' | '\x39' -> Arith16.add_ix_pp (op lsr 4)
  | '\x21' -> LD16.ld_ix_nn (Uint16.to_int (Reader.Read.u16 reader))
  | '\x22' -> LD16.ld_NN_ix (Uint16.to_int (Reader.Read.u16 reader))
  | '\x23' -> Arith16.inc_ix
  | '\x2a' -> LD16.ld_ix_NN (Uint16.to_int (Reader.Read.u16 reader))
  | '\x2b' -> Arith16.dec_ix
  | '\x34' -> Arith8.inc_IXd (Int8.to_int (Reader.Read.i8 reader))
  | '\x35' -> Arith8.dec_IXd (Int8.to_int (Reader.Read.i8 reader))
  | '\x36' ->
      let d = Uint8.to_int (Reader.Read.u8 reader) in
      LD8.ld_IXd_n d (Int8.to_int (Reader.Read.i8 reader))
  ...
  | '\x46' | '\x4e' | '\x56' | '\x5e' | '\x66' | '\x6e' | '\x7e' ->
      LD8.ld_r_IXd ((op lsr 3) land 0b111) (Int8.to_int (Reader.Read.i8 reader))
  | '\x70' .. '\x75' | '\x77' ->
      LD8.ld_IXd_r (Int8.to_int (Reader.Read.i8 reader)) (op land 0b111)
  ...
  | '\x86' | '\x8e' | '\x96' | '\x9e' | '\xa6' | '\xae' | '\xb6' ->
      Arith8.op_a_IXd
        ((op lsr 3) land 0b111)
        (Int8.to_int (Reader.Read.i8 reader))
  | '\xbe' -> Arith8.cp_a_IXd (Int8.to_int (Reader.Read.i8 reader))
...
let decode_ed pc reader =
  ...
  | '\x43' | '\x53' | '\x63' | '\x73' ->
      LD16.ld_NN_dd
        (Uint16.to_int (Reader.Read.u16 reader))
        ((op lsr 4) land 0b11)
...
let decode_fd reader =
  ...
  | '\x26' | '\x2e' ->
      LD8.ld_q_n ((op lsr 3) land 0b111) (Uint8.to_int (Reader.Read.u8 reader))
  | '\x09' | '\x19' | '\x29' | '\x39' -> Arith16.add_iy_qq (op lsr 4)
  | '\x21' -> LD16.ld_iy_nn (Uint16.to_int (Reader.Read.u16 reader))
  | '\x22' -> LD16.ld_NN_iy (Uint16.to_int (Reader.Read.u16 reader))
  | '\x23' -> Arith16.inc_iy
  | '\x2a' -> LD16.ld_iy_NN (Uint16.to_int (Reader.Read.u16 reader))
  | '\x2b' -> Arith16.dec_iy
  | '\x34' -> Arith8.inc_IYd (Int8.to_int (Reader.Read.i8 reader))
  | '\x35' -> Arith8.dec_IYd (Int8.to_int (Reader.Read.i8 reader))
  | '\x36' ->
      let d = Uint8.to_int (Reader.Read.u8 reader) in
      LD8.ld_IYd_n d (Int8.to_int (Reader.Read.i8 reader))
  | '\x44' | '\x45' | '\x4c' | '\x4d' | '\x54' | '\x55' | '\x5c' | '\x5d'
  | '\x60' .. '\x65'
  | '\x67' .. '\x6d'
  | '\x6f' | '\x7c' | '\x7d' ->
  ...
  | '\x46' | '\x4e' | '\x56' | '\x5e' | '\x66' | '\x6e' | '\x7e' ->
      LD8.ld_r_IYd ((op lsr 3) land 0b111) (Int8.to_int (Reader.Read.i8 reader))
  | '\x70' .. '\x75' | '\x77' ->
      LD8.ld_IYd_r (Int8.to_int (Reader.Read.i8 reader)) (op land 0b111)
  | '\x84' | '\x85' | '\x8c' | '\x8d' | '\x94' | '\x95' | '\x9c' | '\x9d'
  | '\xa4' | '\xa5' | '\xac' | '\xad' | '\xb4' | '\xb5' ->
  ...
  | '\x86' | '\x8e' | '\x96' | '\x9e' | '\xa6' | '\xae' | '\xb6' ->
      Arith8.op_a_IYd
        ((op lsr 3) land 0b111)
        (Int8.to_int (Reader.Read.i8 reader))
  | '\xbe' -> Arith8.cp_a_IYd (Int8.to_int (Reader.Read.i8 reader))
...
let decode pc reader =
  ...
  | '\x00' -> GPAC.nop
  | '\x01' | '\x11' | '\x21' | '\x31' ->
      LD16.ld_dd_nn (op lsr 4) (Uint16.to_int (Reader.Read.u16 reader))
  | '\x02' -> LD8.ld_BC_a
  ...
  | '\x06' | '\x0e' | '\x16' | '\x1e' | '\x26' | '\x2e' | '\x3e' ->
      LD8.ld_r_n (op lsr 3) (Uint8.to_int (Reader.Read.u8 reader))
  | '\x07' -> RS.rlca
  | '\x08' -> EBTS.ex_af_af'
  ...
  | '\x0f' -> RS.rrca
  | '\x10' -> JMP.djnz_e pc (Int8.to_int (Reader.Read.i8 reader))
  | '\x12' -> LD8.ld_DE_a
  | '\x17' -> RS.rla
  | '\x18' -> JMP.jr_e pc (Int8.to_int (Reader.Read.i8 reader))
  | '\x1a' -> LD8.ld_a_DE
  | '\x1f' -> RS.rra
  | '\x20' | '\x28' | '\x30' | '\x38' ->
      JMP.jr_ss_e pc
        ((op lsr 3) land 0b11)
        (Int8.to_int (Reader.Read.i8 reader))
  | '\x22' -> LD16.ld_NN_hl (Uint16.to_int (Reader.Read.u16 reader))
  | '\x27' -> GPAC.daa
  | '\x2a' -> LD16.ld_hl_NN (Uint16.to_int (Reader.Read.u16 reader))
  | '\x2f' -> GPAC.cpl
  | '\x32' -> LD8.ld_NN_a (Uint16.to_int (Reader.Read.u16 reader))
  | '\x34' -> Arith8.inc_HL
  | '\x35' -> Arith8.dec_HL
  | '\x36' -> LD8.ld_HL_n (Uint8.to_int (Reader.Read.u8 reader))
  | '\x37' -> GPAC.scf
  | '\x3a' -> LD8.ld_a_NN (Uint16.to_int (Reader.Read.u16 reader))
  | '\x3f' -> GPAC.ccf
  | '\x40' .. '\x45'
  | '\x47' .. '\x4d'
  | '\x4f' .. '\x55'
  ...
  | '\xc2' | '\xca' | '\xd2' | '\xda' | '\xe2' | '\xea' | '\xf2' | '\xfa' ->
      JMP.jp_cc_nn pc
        ((op lsr 3) land 0b111)
        (Uint16.to_int (Reader.Read.u16 reader))
  | '\xc3' -> JMP.jp_nn (Uint16.to_int (Reader.Read.u16 reader))
  ...
  | '\xc6' | '\xce' | '\xd6' | '\xde' | '\xe6' | '\xee' | '\xf6' ->
      Arith8.op_a_n
        ((op lsr 3) land 0b111)
        (Uint8.to_int (Reader.Read.u8 reader))
  | '\xfe' -> Arith8.cp_a_n (Uint8.to_int (Reader.Read.u8 reader))
  | '\xc7' | '\xcf' | '\xd7' | '\xdf' | '\xe7' | '\xef' | '\xf7' | '\xff' ->
      RTN.rst_p pc ((op lsr 3) land 0b111)
  | '\xc9' -> RTN.ret
  | '\xcb' -> decode_cb reader
  | '\xcd' -> RTN.call pc (Uint16.to_int (Reader.Read.u16 reader))
  | '\xd3' -> IO.out_N_a (Uint8.to_int (Reader.Read.u8 reader))
  | '\xd9' -> EBTS.exx
  | '\xdb' -> IO.in_a_N (Uint8.to_int (Reader.Read.u8 reader))
...
let decode reader vaddr =
  let lo = Reader.get_pos reader in
  let disasm = decode vaddr reader in
  let size = Reader.get_pos reader - lo in
  Reader.set_pos reader lo;
  let opcode = String_utils.to_hex (Reader.Read.bytes reader size) in
  let mnemonic, dhunk =
    match disasm with
    | F (mnemonic, instrs) ->
...
let () = Decoder.register Machine.z80 decode
