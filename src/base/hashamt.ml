...
module Make (Key : HashedType) : S with type key = Key.t = struct
  ...
  module Bucket = struct
    ...
    let union f b b' : 'a t =
      List.fold_left
        (fun u ((k, _) as a) -> if List.mem_assoc k b then u else a :: u)
        (List.fold_left
           (fun u ((k, v) as a) ->
             try
  ...
  module Knot = struct
    ...
    let popcnt i =
      let i =
        (i land 0x5555555555555555) + ((i lsr 1) land 0x5555555555555555)
      in
      let i =
        (i land 0x3333333333333333) + ((i lsr 2) land 0x3333333333333333)
      in
      let i =
    ...
    let mem x b : bool = b land select x != 0 [@@inline]
    let get x v b : 'a = v.(popcnt (b land (mask) x)) [@@inline]
    let set x a v b : unit = v.((popcnt) (b land (mask) x)) <- a [@@inline]
  ...
  let rec recadd d k a h t =
    ...
    | Item (k', a', h') ->
        if h = h' then
          if Key.equal k k' then Item (k, a, h)
          else Bucket (Bucket.add k' a' [ (k, a) ], h)
        else
          let i' = prefix land (h' lsr d) in
          recadd d k a h (Knot.single i' t)
    | Bucket (l, h') ->
        if h = h' then Bucket (Bucket.add k a l, h)
        else
          let i' = prefix land (h' lsr d) in
          recadd d k a h (Knot.single i' t)
    | Knot (v', b') ->
  ...
  let rec recremove d k h t =
    match t with
    | Empty -> Empty
    | Item (k', _, h') -> if h = h' && Key.equal k k' then Empty else t
    | Bucket (l', h') ->
        if h = h' then
    ...
    | Knot (v', b') ->
        ...
        if Knot.mem i b' then
          ...
          | Empty ->
              ...
              else if Array.length v' = 2 then
                let i' = 1 - Knot.popcnt (b' land Knot.mask i) in
                let u = v'.(i') in
                match u with Item _ | Bucket _ -> u | _ -> Knot.remove i v' b'
              else Knot.remove i v' b'
          | Item _ | Bucket _ ->
              if Array.length v' = 1 then t'' else Knot.update i t'' v' b'
  ...
  let rec recunion d f t t' =
    ...
    | Item (k, a, h), Item (k', a', h') ->
        if h = h' && Key.equal k k' then
          match f k a a' with None -> Empty | Some a'' -> Item (k, a'', h)
        else recadd d k a h t'
    | Bucket (b, h), Item (k', a', h') ->
        if h = h' && Bucket.mem k' b then
          recunion d f t (Bucket ([ (k', a') ], h'))
        else recadd d k' a' h' t
    | Bucket (b, h), Bucket (b', h') ->
        if h = h' then Bucket (Bucket.union f b b', h)
        else
          let i' = prefix land (h' lsr d) in
        ...
        if s > 3 * size / 4 then (
          ...
          for i = 0 to size - 1 do
            if Knot.mem i b && Knot.mem i b' then
              match
                recunion (d + shift) f (Knot.get i v b) (Knot.get i v' b')
              with
              | Empty -> ()
              | t'' -> Knot.set i t'' v'' b''
            else if Knot.mem i b then Knot.set i (Knot.get i v b) v'' b''
            else if Knot.mem i b' then Knot.set i (Knot.get i v' b') v'' b''
          done;
          Knot (v'', b'')
  ...
  let rec recjoin d f t t' =
    ...
    | Bucket (b, h), Bucket (b', h') ->
        if h = h' then Bucket (Bucket.join f b b', h) else Empty
    | Knot (v, b), Item (_, _, h') | Knot (v, b), Bucket (_, h') ->
        let i' = prefix land (h' lsr d) in
        if Knot.mem i' b then
    ...
    | Knot (v, b), Knot (v', b') ->
        ...
        if !s > 3 * size / 4 then Node (v'', !s)
        else if !s > 0 then Knot.compact v'' !s
        else Empty
    ...
    | Node (v, _), Knot (v', b') ->
        ...
        if !s > 3 * size / 4 then Node (v'', !s)
        else if !s >= 1 then Knot.compact v'' !s
        else Empty
    | Node (v, _), Node (v', _) ->
        let v'' = Array.make size Empty in
        let s = ref 0 in
