          ...
          | BvEqual -> (
              ...
              | BvBnop (BvConcat, bv1', bv2') ->
                  ...
                  mk_bl_and
                    (mk_bv_equal bv1'
                       (mk_bv_cst
                          (Bitvector.extract ~hi:(sz1 + sz2 - 1) ~lo:sz2 bv)))
                    (mk_bv_equal bv2'
                       (mk_bv_cst (Bitvector.extract ~hi:(sz2 - 1) ~lo:0 bv)))
              | BvIte (bl, bv1, bv2) -> (
                  match (is_bv_cst bv1, is_bv_cst bv2) with
  ...
  | BvUnop (u, bv) -> (
      ...
      | Some bv ->
          ...
          mk_bv_cst
            ...
            | BvNot -> lognot bv
            | BvNeg -> neg bv
            | BvRepeat i -> concat (List.init i (Fun.const bv))
            ...
            | BvExtract { hi; lo } -> extract ~hi ~lo bv))
  | BvBnop (b, bv1, bv2) -> (
      ...
      | None, Some bv -> (
          ...
          | BvAnd -> (
              if Bitvector.is_zeros bv then bv2
                ...
                | BvBnop (BvConcat, bv1, bv2) ->
                    let b1 =
                      Bitvector.extract ~hi:(bv_term_size - 1)
                        ~lo:bv2.bv_term_size bv
                    in
                    let b2 =
                      Bitvector.extract ~hi:(bv2.bv_term_size - 1) ~lo:0 bv
                    in
...
let mk_bv_extract i bv =
  ...
  if not (i.hi >= i.lo && bv.bv_term_size > i.hi && i.lo >= 0) then
    Formula_logger.fatal "invalid extract of bits [%i;%i] for bv %a of size %i"
      i.lo i.hi Printing.p_bvterm bv bv.bv_term_size;
