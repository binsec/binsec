...
module ReadOverWrite = struct
  ...
  module ItvEnv : S = struct
    ...
    let lookup t interval n ax bv =
      ...
      match overlap with
      | _ :: _ ->
          Formula_logger.debug "LOOKUP CONFLICT\n";
          mk_select n ax bv
      | [] -> (
          match opt with
      ...
      | Store (n, array, bv1, bv2) ->
          ...
          AxTermHashtbl.add t ax
            ( (if overlap = [] then arr
                 ...
                 Formula_logger.debug "UPDATE CONFLICT\n";
                 array)),
  ...
  let assert_interval t bl =
    ...
    | None -> (
        ...
        | BvComp (BvEqual, bv1, bv2) ->
            ...
            in
            if is_empty itv then Formula_logger.warning "Empty domain";
