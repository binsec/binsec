...
module Memory = struct
  ...
  let pp_with_sections section_name ppf t =
    let open Format in
    if is_empty t then pp_print_string ppf "-- empty memory --"
      ...
      List.iter
        (fun (address, value) ->
          let name = section_name (Virtual_address.of_bitvector address) in
  ...
  let pp_variables ppf r =
    if Stbl.length r > 0 then (
      ...
      List.iter
        ...
        l;
      pp_print_cut ppf ())
  and pp_memory ppf m = Memory.pp_with_sections section_name ppf m.memory in
...
let pp = pp_with_sections (Fun.const None)
...
let value_of_constant cst =
  match cst with
  | CstBool false -> Bitvector.zero
  | CstBool true -> Bitvector.one
  | CstHexadecimal s ->
      Bitvector.create (Z.of_string_base 16 s) (4 * String.length s)
  | CstBinary s -> Bitvector.create (Z.of_string_base 2 s) (String.length s)
...
let extract_value v = value_of_constant v
...
let yices_extract (raw : string) =
  ...
  let parse_line (smt_model, memory_name) raw_line =
    ...
    in
    let line = String.split_on_char ' ' raw_line in
    match line with
