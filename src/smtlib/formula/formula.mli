...
(** Definition of abstract representation for logical formulas (based on
    SMTLIB's syntax and semantics) *)
...
(** An user defined entry.
    ...
    Custom entry support is experimental and very limited. For now, it will work
    only for formula creation (no transformation) and printing (and by
    extension, call to external SMT solver). *)
...
(** [register_custom { hash; pp }] registers a hash and a pretty printer
    functions for custom entries.

    The hash function should return [Some h] for the custom entries the handler
    supports, [None] otherwise. The pretty printer should print the entry
    without the surrounding parenthesis and return [true] for the custom entries
    the handler supports, [false] otherwise.
...
(** An exception raised by a function that can not handle a custom entry, either
    because no hander has been registered for this entry or because the function
    does not support entry extension. *)
