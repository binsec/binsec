...
let print_bv_unop = Format.asprintf "%a" pp_bvunop
let print_bv_bnop = Format.asprintf "%a" pp_bvbnop
let print_bv_comp = Format.asprintf "%a" pp_bvcomp
...
let pp_formula ppf fm =
  ...
  (Format.pp_print_list ~pp_sep:Format.pp_print_space pp_entry)
    ppf
    (Sequence.fold_backward (fun en acc -> en :: acc) fm.entries []);
  Format.pp_close_box ppf ()

let print_bv_term e = Format.asprintf "%a" pp_bv_term e
let print_ax_term e = Format.asprintf "%a" pp_ax_term e
let print_bl_term e = Format.asprintf "%a" pp_bl_term e
...
let ax_to_string name size1 size2 =
  Printf.sprintf "(declare-fun %s () (Array (_ BitVec %i) (_ BitVec %i)))" name
    size1 size2

let pp_varset ppf set =
  VarSet.iter
    (function
      | BlVar bl -> Format.fprintf ppf "%s;@ " (bl_to_string bl.bl_name)
      | BvVar bv ->
          Format.fprintf ppf "%s;@ " (bv_to_string bv.bv_name bv.bv_size)
      | AxVar ax ->
          Format.fprintf ppf "%s;@ "
            (ax_to_string ax.ax_name ax.idx_size ax.elt_size))
    set

let print_varset set = Format.asprintf "@[<v 0>%a@]" pp_varset set

let pp_header ~theory (* ?(theory = "QF_ABV") *) ppf () =
  fprintf ppf "@[<v 0>(set-logic %s)@ (set-info :smt-lib-version 2.0)@ @]"
    theory

let print_header ~theory () = Format.asprintf "%a" (pp_header ~theory) ()
