...
module type TERM = sig
  module rec Bl : sig
    ...
    *)
    ...
    val ite : t -> t -> t -> t
    (** [ite x y z] creates the boolean [ite] operation between [y] and [z]
        according to [x]. *)
  ...
  and Bv : sig
    ...
    (** [const size name] creates the (first-order) constant of [size] bits
        named [name] . *)

    val value : int -> Z.t -> t
    (** [value sz bv] creates a constant from the bitvector value [bv] of size
        [sz]. *)
    ...
    val lognand : t -> t -> t
    (** [lognand x y] creates the [nand] bitvector operation between [x] and
        [y]. *)
    ...
    *)
    ...
    val logxnor : t -> t -> t
    (** [logxnor x y] creates the [xnor] bitvector operation between [x] and
        [y]. *)
    ...
    val append : t -> t -> t
    (** [append x y] creates the concatenation of the two bitvectors [x] and
        [y]. *)

    val extract : hi:int -> lo:int -> t -> t
    (** [extract ~hi ~lo x] extracts the sub-bitvector of [x] starting from bit
        [lo] and ending at bit [hi] (included). *)

    val uext : int -> t -> t
    (** [uext n x] zero-extends the bitvector [x] by adding [n] bits. *)

    val sext : int -> t -> t
    (** [sext n x] sign extends the bitvector [x] copying [n] times the most
        significant bit. *)

    val ite : Bl.t -> t -> t -> t
    (** [ite x y z] creates the bitvector [ite] operation between [y] and [z]
        according to [x]. *)
  ...
  and Ax : sig
    ...
    (** [sort ~idx elm] creates a new array kind that maps [idx]-bit bitvector
        indexes to [elm]-bit bitvector values. *)
    ...
    *)
    ...
    (** [store a i x] creates the array [store] operation of the byte [x] at
        index [i] in the array [a]. *)

    val select : t -> Bv.t -> Bv.t
    (** [select a i] creates the array [select] operation of one byte at index
        [i] in the array [a]. *)
    ...
    val ite : Bl.t -> t -> t -> t
    (** [ite x y z] creates the array [ite] operation between [y] and [z]
        according to [x]. *)
  end
end
...
module type S = sig
  ...
  val pop : unit -> unit
  (** [pop ()] discard all the assertions since the last backup point, restoring
      the solver context in the same state as before the [push ()]. Invalid uses
      may fail in an unpredictable fashion. *)
  ...
  (** [check_sat_assuming e] checks if the current formula is satisfiable with
      the assumtion [e].

      @return Sat, Unsat or Unknown. *)

  val get_bv_value : Bv.t -> Z.t
  (** [get_bv_value expr] returns the assignment of the expression [expr] if
  ...
  (** [fold_ax_values f ax v] iter through the assignment of the array [ax] if
      [check_sat] returned [Sat]. Invalid uses may fail in an unpredictable
      fashion.
