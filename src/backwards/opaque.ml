...
module Opacity_status = struct
  type t = {
    c_op : Opacity.t;
    (* Opacity of consequent *)
    a_op : Opacity.t; (* Opacity of alternative *)
  }

  let default = { c_op = Opacity.Unknown; a_op = Opacity.Unknown }
  ...
  let pp ppf st =
    fprintf ppf "@[<hov>then: %a;@ else: %a@]" Opacity.pp st.c_op Opacity.pp
      st.a_op
  ...
  *)
  let combine opa1 opa2 =
    ...
    | _, Clear | Clear, _ -> Clear
    | Unknown, o | o, Unknown -> o
    | Opaque, Opaque -> Opaque

  let ( <+> ) st1 st2 =
    { c_op = combine st1.c_op st2.c_op; a_op = combine st1.a_op st2.a_op }
end
...
module Trace = struct
  ...
  let pop = function x :: xs -> (x, xs) | [] -> failwith "empty trace"

  let peek = function x :: _ -> x | [] -> failwith "empty trace"
  ...
  let pp ppf t =
    ...
    iter
      (fun i ->
        fprintf ppf "%a: %a;@ " Virtual_address.pp i.Instruction.address
...
module Translator = struct
  ...
  let create ?f tr =
    ...
    let state = match f with None -> state | Some g -> g state in
  ...
  let add_comment pp v t =
    ...
    in
    Sse_symbolic.State.comment cmt_str t.state

  let add_mnemonic inst t = add_comment Mnemonic.pp inst.Instruction.mnemonic t

  let add_assert_constraint t e =
    let assertion, state = mk_assert t.state e in
    let state = Sse_symbolic.State.constrain assertion state in
    { t with state }
  ...
  let independant_formula t = Script (None, genr_formula t)
  ...
  let get_succs t = Dhunk.succ t.inst.Instruction.dba_block t.hidx
  ...
  let rec get_dba t =
    ...
    | None -> (
        ...
        | [ hidx ] -> get_dba { t with hidx }
        | _ -> Logger.fatal "Too many successors")

  let pp ppf t =
    fprintf ppf
      "@[<v 0>Trace@ @[<h>Node: %d@]@ @[<hov 0>Instruction:@ %a@]@ @[<hov \
       0>Next:@ %a@] @]"
      (Dhunk.Node.id t.hidx) Instruction.pp t.inst Trace.pp t.next
  ...
  let next_is_addr addr t =
    ...
    Logger.debug "@[<v 0>Jumping to %a -> next in trace %a@ %a@]"
      Virtual_address.pp vaddr Virtual_address.pp next_inst.Instruction.address
      pp t;
  ...
  module Eval = struct
    ...
    let ite e jt _next_id t =
      ...
      | Dba.JOuter addr ->
          let t =
            if next_is_addr addr t then add_assert_constraint t e
            else add_deny_constraint t e
          in
          step t
  end
    ...
    let rec loop res =
      try
        ...
        | Continue t ->
            ...
            Logger.debug ~level:3 "Trace %@ %a" Dba_printer.Ascii.pp_instruction
              dba;
            do_dba t dba
      with e -> (* t, None *) raise e
    and do_dba t dba =
      let t =
        { t with state = add_comment Dba_printer.Ascii.pp_instruction dba t }
      in
      match dba with
      ...
      | Assignment (lval, _, _) ->
          let msg =
            Format.asprintf "Unhandle deterministc assigment for %a"
              Dba_printer.EICAscii.pp_lhs lval
          in
          failwith msg
      | Mem_load (addr, size) ->
          let addr =
      ...
      | Assumption e ->
          let msg =
            Format.asprintf "Unhandled assumption %a"
              Dba_printer.EICAscii.pp_bl_term e
          in
          failwith msg
    in
    List.fold_left f symb_st initials
...
let check_trace c tr =
  ...
  | t, Some _ ->
      ...
      let c_sat =
        solve ~title:"consequent" Translator.(add_assert_constraint t e)
      and a_sat =
        solve ~title:"alternative" Translator.(add_deny_constraint t e)
      in
      let open Opacity_status in
      let a_op = Opacity.of_sat a_sat and c_op = Opacity.of_sat c_sat in
      { a_op; c_op }
  | exception Translator.Eval.Broken_static_jump ->
      Logger.warning
        "Could not determine status for instruction: defaulting ...";
...
let backward_build_traces max_sz cfg v =
  let rec loop tr sz v =
    if sz >= max_sz then [ tr ]
      ...
      let tr' =
        match IC.V.inst v with None -> tr | Some i -> Trace.cons i tr
      in
...
module Check = struct
  ...
  let predicate ~cfg c v =
    ...
    in
    loop Opacity_status.default traces
  ...
  let instruction ~cfg inst v =
    ...
    | None -> Ok None
    | Some c -> (
        match predicate ~cfg c v with Ok s -> Ok (Some s) | Error e -> Error e)

  let vertex ~cfg v =
    match IC.V.inst v with
    | None -> None
    | Some inst -> (
        Logger.debug ~level:4 "@[<h>%@%a@ :@ %a@]" Virtual_address.pp
        ...
        | Ok (Some ost as r) ->
            Logger.result "@[@[<h>Predicate %a@ %@ 0x%a is %a@]@ (%a)@]"
              ...
              Opacity_status.pp_combined ost Opacity_status.pp ost;
            r
        | Error _ ->
            Logger.warning "Could not evaluate predicate %a %@ 0x%a" Mnemonic.pp
              (Instruction.mnemonic inst)
              Virtual_address.pp (Instruction.address inst);
            None)
  ...
  let addresses ~cfg vaddrs =
    List.fold_left
      (fun map va ->
        ...
        | None -> map
        | Some v ->
            Logger.debug "@[<h>Checking opacity %@%a@]" Virtual_address.pp va;
            add_map map (IC.V.addr v) (vertex ~cfg v))
      Virtual_address.Map.empty vaddrs
