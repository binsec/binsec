...
let instr_succ addr instr =
  ...
  match instr with
  | Assign _ | Stop _ | Assert _ | Assume _ | NondetAssume _ | Nondet _
  | Undef _ | Free _ | Malloc _ | Print _ ->
      Some [ Dba.JInner (addr.Dba.id + 1) ] (* no jump *)
  ...
  if G.mem_vertex_a cfg root_addr = None then (
    ...
    Dhunk.iteri
      ~f:(fun index dba_instr ->
        ...
        if depth - index >= 0 then (
          G.add_inst cfg addr dba_instr;
          let targets =
            match instr_succ addr dba_instr with None -> [] | Some x -> x
          in
          List.iter
            (function
              | Dba.JInner id ->
                  if depth - id >= 0 then G.add_edge_a cfg addr (i_to_a id)
              | Dba.JOuter a ->
                  if not stop then (
                    populate_from cfg
                      (Dba_types.Caddress.to_virtual_address a)
                      (depth - index - 1);
                    if depth - index - 1 >= 0 then (
