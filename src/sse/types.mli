...
(** The reason a path execution ends (via {!constructor-Signal}). *)
...
type 'path continuation =
  ...
  | Return_to of ([ `All ], 'path) fiber
      (** Return from a {!constructor-Call} but reroutes the execution to the
          given low-level instruction instead of the {!constructor-Call}
          continuation. *)
  | Signal of status
      (** Ends the path execution.

          If the status is {!constructor-Stashed}, the excecution can be resumed
          with the function {!module-type-ENGINE.val-resume} or a
          {!constructor-Merge} request. *)
  | Fork of 'path continuation * 'path continuation
      (** Request for a path fork. The parent path will evaluate the first
          continuation while the child path will evaluate the second one. *)
  | Merge of 'path list * 'path continuation
      (** Request for a path merging. The symbolic engine will try to merge the
          current path with the given ones.

          All the listed paths must have been paused with {!constructor-Signal}
          {!constructor-Stashed}.

          The merge result will evaluate the given continuation. If a path can
          not be merged, it will evaluate the given continuation too. *)
...
(** A hook declaration as used by the {!type-extension}s.
    ...
    Within the same phase, {!constructor-Early} stage hooks are evaluated before
    {!constructor-Late} stage hooks.

    Within the same stage, hooks are evaluated in the order of insertion.

    A hook is only active under a given [scope] (i.e range of addresses or
    [None] for {i always}). *)
...
type 'path extension =
  | Initialization_callback of ('path -> unit)
      (** Registers the function [f] to be called during initialization of the
          first [path].

          It can be used to set the initial value of some [Dba] variables in the
          state or initialize some fields in the [path] structure.

          The function [f] is called only once and before any instruction in the
          script. *)
  | Fetch_hook of (Virtual_address.t -> Ir.Graph.t option) hook
      (** Registers the {!type-hook} [h] to be called each time a new virtual
          address in the [scope] is discovered.

          Given the virtual address, the [callback] can return a new instruction
          graph ({!module-Ir.module-Graph.type-t}) to hook this address.
          ...
          Otherwise, the hook acts as a replacement and skips the subsequent
          ones.

          The hooks of the same stage are evaluated in the same order they are
          inserted. *)
  | Decode_hook of (Virtual_address.t -> int Reader.t -> Ir.Graph.t option) hook
      (** Registers the {!type-hook} [h] to be called each time a new virtual
          address in the [scope] is about to be decoded (after all fetch hooks).

          Given the virtual address and the byte opcode
          ({!module-Reader.type-t}), the [callback] can return a new instruction
          graph ({!module-Ir.module-Graph.type-t}) to hook this address.
  ...
  | Disasm_hook of (Instruction.t -> Ir.Graph.t option) hook
      (** Registers the {!type-hook} [h] to be called each time a new virtual
          address in the [scope] has been decoded (after all decode hooks).

          Given the decoded instruction ({!module-Instruction.type-t}), the
          [callback] can return a new instruction graph
  ...
  | Rewrite_hook of (Ir.Graph.t -> unit) hook
      (** Registers the {!type-hook} [h] to be called each time a new virtual
          address in the [scope] is about to yield a instruction graph
          ({!module-Ir.module-Graph.type-t}) (after all disasm hooks).

          Given the instruction graph, the [callback] can modify it in place
          using the {!module-Ir.module-Graph} interface (e.g.
  ...
  | Instrumentation_routine of (Revision.t -> unit)
      (** Registers the funcion [f] to instrument the intermediate
          representation ({!module-Ir}).

          It can be used to automatically insert new micro-instructions,
          including custom {!module-Ir.type-builtin}s at strategic points (e.g.
          memory access, branch conditions, etc.).

          The function [f] is called each time the engine discover new assembly
          instructions. The function [f] can explore the new micro-instructions
          by using the {!module-Revision} interface. It can add new
          micro-instructions with the functions
          {!module-Revision.val-insert_before} and
          {!module-Revision.val-insert_list_before}.

          The function should only modify the newly disassembled part of the
          graph ({!module-Revision.val-is_new_vertex},
          {!module-Revision.val-iter_new_vertex}). Inserting instructions in a
          previously visited part will raise {!exception-Invalid_argument}.

          The function should not insert instructions before an
          {!module-Ir.constructor-Instruction} label. To instrument the
          {!module-Ir.constructor-Instruction} labels, use the
          {!module-Revision.val-insert_before} or
          {!module-Revision.val-insert_list_before} function on the successor of
          the node. *)
  ...
  | Instruction_resolver of
      ...
      (** Registers the handler [f] to translate custom script instructions
          ...
          The handler [f] can use the parser environment
          ({!module-Script.type-env}) to parse and resolve script expressions
          with {!module-Script.val-eval_loc} and {!module-Script.val-eval_expr}.
  ...
  | Command_handler of (Script.Ast.t -> Script.env -> 'path -> bool)
      (** Registers the handler [f] to perform custom top level script commands
          ...
          The handler [f] should return [true] when the command has been
  ...
  | Builtin_resolver of (Ir.builtin -> 'path primitive)
      (** Registers the function [f] in charge to resolve the handler of a given
          ...
          The handler [h] has access to the [path] interface
          ({!module-type-PATH}).

          It should return {!constructor-Return} to resume the exploration along
          this path. {!constructor-Signal}[ status] cuts the path and logs the
          given status.
  ...
  | Builtin_may_read of (Ir.builtin -> Dba_types.Var.Set.t option option)
      (** Registers the knowledge provider [f] for custom
          ...
          It must return [Some k] for the [builtin] it handles, [None]
          otherwise.
          - [k = Some set] is the (overapproximated) set of all variables the
            builtin may access. It is an error to access a variable not listed
            here and may result in undefined value.
  ...
  | Builtin_may_write of (Ir.builtin -> Dba_types.Var.Set.t option option)
      (** Registers the knowledge provider [f] for custom
          ...
          otherwise.
          - [k = Some set] is the (overapproximated) set of all variables the
            builtin may modify. It is an error to modify a variable not listed
            here and may result in undefined value.
  ...
  | Builtin_must_write of (Ir.builtin -> Dba_types.Var.Set.t option)
      (** Registers the knowledge provider [f] for custom
          ...
          It must return [Some set] for the [builtin] it handles, [None]
          otherwise. [set] is the (underapproximated) set of the variables a
          builtin must overwrite. Previous values of these variables are deemed
          no longer reachable. *)
  | Builtin_printer of (Format.formatter -> Ir.builtin -> bool)
      (** Registers the custom printer [f] to format the custom
          {!module-Ir.type-builtin}s in debug outputs.

          The printer [f] should return [true] when the
          {!module-Ir.type-builtin} is treated, [false] otherwise. *)
  | Fork_callback of ('path -> 'path -> unit)
      (** Registers the function [f] to be called when a path execution is about
          to fork.

          It can be used to maintain invariants between the parent (first
          parameter) and the child (second parameter) paths. *)
...
(** An identifier used to declare new field in the {!module-type-PATH} structure
    ...
    is present in one {!module-type-PLUGIN.val-fields} list. The type ['a] can
    be anything, including a function, and may depend on the type parameters
    ['value], ['model], ['state] and ['path].
...
(** A field declaration used to declare new field in the {!module-type-PATH}
    ...
    The declaration contain an identifier [id] ({!type-field_id}), the [default]
    value, an optional [copy] function and an optional [merge] function.

    The [copy] function is called each time a path is forked to create a new
    local copy of the field value (by {b default} {!module-Fun.val-id}).

    The [merge] function is called each time two paths are about to merge. It
    returns [Some] when the field values are mergeable, and [None] otherwise (by
    {b default}, it returns [Some] if both values are physically equal). *)
and ('value, 'model, 'state, 'path) field =
  | Field : {
...
module type ENGINE = sig
  ...
  (** [fs filepath] returns the content of [filepath]. It uses the virtual file
      system which may differ from the host one.
  ...
  module Debug : sig
    val symbol : ?info:Dba.Var.Tag.attribute -> string -> Z.t option
    (** [symbol ~info name] returns the value of the queried [info] attribute of
        the symbol [name] (by {b default},
        {!module-Dba.module-Var.module-Tag.contructor-Value}). *)
...
module type PLUGIN = sig
  ...
  (** [fields path] should return a list of custom fields
      ([[ field0; field1; ... ]]) to register into the path structure.

      The associated {!module-type-PATH.type-key} can be retrieved via the
      function {!module-type-ENGINE.val-lookup}. *)
  ...
  (** [extensions engine] should return a list of custom extentions to activate
      ...
      This function is called only once per symbolic execution run and comes
      after {!val-fields}. *)
end
