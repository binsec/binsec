...
let parse_model session =
  ...
  | Yices -> Smt_model.yices_extract raw_model
...
let start_interactive ?file ?(timeout=0) solver =
  ...
  Logger.debug ~level:4 "Starting session for %s" (Prover.name_of solver);
  if file <> None then
    Logger.debug ~level:4 "Session transcript written to file@ %s"
      (Utils.unsafe_get_opt file);
  let ppf = get_formatter_of_session session in
  Format.pp_print_string ppf (print_header ());
