...
module Memory = Types.Memory
module StTbl = Types.StTbl
module BvTbl = Types.BvTbl
module AsTbl = Types.AsTbl
module AxTbl = Types.AxTbl
...
module Context (H : AXHOOK) (S : Smtlib.Bindings.S) :
...
struct
  ...
  and visit_bv bv =
    ...
    with Not_found ->
      let e =
        ...
        | Binary { f = Urem; x; y; _ } -> Bv.urem (visit_bv x) (visit_bv y)
        | Binary { f = Srem; x; y; _ } -> Bv.srem (visit_bv x) (visit_bv y)
  ...
  and visit_ax (ax : Memory.t) =
    ...
    with Not_found ->
      let a =
        match ax with
        | Symbol { name; index; _ } as symbol ->
            StTbl.add farrays name symbol;
            Ax.const (Ax.sort ~idx:index byte_size) name
        | Layer { addr = Cst _ as addr; store; over = Symbol _ as symbol; _ } ->
            let idx_size = Expr.sizeof addr in
            let base = visit_bv addr and array = visit_ax symbol in
            Store.fold
              (fun i chunk array ->
                if Chunk.is_hunk chunk then array
                else
                  visit_store idx_size base i (Chunk.unsafe_to_term chunk) array)
              array store
        | Layer { addr; store; over; _ } ->
            let base = visit_bv addr and array = visit_ax over in
            Store.fold_term (visit_store (Expr.sizeof addr) base) array store
      in
      H.visit_ax ax;
      AxTbl.add ax_cons ax a;
      a

  and visit_store idx_size base i value array =
    let s = Expr.sizeof value in
    let x = visit_bv value and index = Bv.add base (Bv.value idx_size i) in
...
module Make (F : Smtlib.Bindings.OPEN) = struct
  module Open () : Session.S = struct
    module Solver = F ()
    ...
    let fold_array_values :
     ...
     fun f (Symbol _ as ar) x -> Solver.fold_ax_values f (Context.visit_ax ar) x
...
module SafeArray (F : Smtlib.Bindings.OPEN) = struct
  ...
  *)
  ...
  module Open () : Session.S = struct
    ...
    let ax_root : Memory.symbol AxTbl.t = AxTbl.create 64
    let ordered_mem : access Queue.t AsTbl.t = AsTbl.create 4

    module Hook = struct
      let visit_load
          (Load { len; addr; label; _ } : ([ `Mem ], string, Memory.t) Term.t) =
        let root = AxTbl.find ax_root label in
        let ordered_mem = AsTbl.find ordered_mem root in
      ...
      let visit_ax (ax : Memory.t) =
        ...
        | Symbol _ as ax ->
            ...
            AsTbl.add ordered_mem ax (Queue.create ())
        | Layer { addr = Cst _ as addr; store; over = Symbol _ as symbol; _ } ->
            AxTbl.add ax_root ax symbol;
            let ordered_mem = AsTbl.find ordered_mem symbol in
            Store.iter
              (fun i chunk ->
                if Chunk.is_term chunk then
                  Queue.push
                    (Store
                       ( Expr.addz addr i,
                         Expr.sizeof (Chunk.unsafe_to_term chunk) lsr 3 ))
                    ordered_mem)
              store
        | Layer { addr; store; over; _ } ->
    ...
    let fold_array_values :
        (Z.t -> Z.t -> 'a -> 'a) -> Memory.symbol -> 'a -> 'a =
     fun f (Symbol _ as ar) x ->
      match AsTbl.find ordered_mem ar with
                        ...
                        let k =
                          get_value
                            (Expr.load 1 LittleEndian
                               (Expr.constant
                                  (Bitvector.create index (Expr.sizeof addr)))
                               ar)
                        in
